<div class="wpjb wpjr-page-my-home">
    <?php $incluyemeCheckIcons = ["wpjb-icon-credit-card", "wpjb-icon-users", "wpjb-box-mymembership", "wpjb-icon-users"];
    $incluyemeCheckSlugs = ["Membresía", "Historial de pagos"]; ?>
    <?php wpjb_flash() ?>
    ...
    <?php foreach ($dashboard as $gname => $group): ?>
		<div class="wpjb-boxes">
            ...
            <?php foreach ($group["links"] as $lname => $link): ?>
                <?php if (!in_array($link["icon"], $incluyemeCheckIcons) && !in_array($link["title"], $incluyemeCheckSlugs)) { ?>
					<a class="wpjb-box wpjb-layer-inside <?php echo "wpjb-box-$lname" ?>"
					   href="<?php esc_attr_e($link["url"]) ?>">
						<span class="wpjb-box-icon wpjb-glyphs <?php esc_attr_e($link["icon"]) ?>"></span>
						<span class="wpjb-box-title"><?php echo esc_html($link["title"]) ?></span>
					</a>
                <?php } ?>
            <?php endforeach; ?>
		</div>
